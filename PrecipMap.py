...
def main():
    ...
    shapefiles = {key: ShapelyFeature(Reader(fname).geometries(), ccrs.PlateCarree(), facecolor='none') for key, fname in shapefiles.items()}

    # Parse XML data
    tree = ET.parse('NDFD_Data.xml')
    root = tree.getroot()
    ...
    # Add logos
    ax.add_artist(AnnotationBbox(OffsetImage(logo1, zoom=0.03), (0.1, 0.1), xycoords='axes fraction', frameon=False, zorder=5))
    ax.add_artist(AnnotationBbox(OffsetImage(logo2, zoom=0.03), (0.25, 0.1), xycoords='axes fraction', frameon=False, zorder=5))
